document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!testForm) {
        console.error("Word translation choice test form ('test-form') not found!");
        return;
    }

    const testDbId = testForm.dataset.testDbId;
    const testTypeSlug = testForm.dataset.testTypeSlug;
    ...
    if (!questionsContainer || !loadingMessage || !noWordsMessageContainer || !timerDisplay || !progressBar || !prevButton || !nextButton || !submitButton) {
        console.error("One or more critical elements for the word translation choice test are missing.");
    ...
    function hideControls() {
        ...
        if(timerDisplay && !isTeacherPreview) timerDisplay.textContent = "Тест не может быть загружен.";
    ...
    if (window.antiCheatInitialCheckPassed === true) {
        ...
        console.warn("Anti-cheat initial check did not pass for word_translation_choice test. Test data will not be loaded.");
    ...
    async function loadAndRenderQuestions() {
        ...
        try {
            const apiUrl = `/api/test/${testDbId}/${testTypeSlug}_words`;
            ...
            if (data.words && data.words.length > 0) {
                ...
                testWordsData.forEach((wordData, index) => {
                    renderQuestion(wordData, index);
                    renderProgressBarItem(index);
                });

                showQuestion(0);
                initializeTimerAndSubmit();
            ...
            console.error("Failed to load or render word translation choice questions:", error);
            if (loadingMessage) loadingMessage.style.display = 'none';
    ...
    function renderQuestion(wordData, index) {
        const questionItem = document.createElement('div');
        questionItem.className = 'word-container question-item';
    ...
    function handleKeyboardNavigation(event) {
        ...
        switch (event.key) {
            ...
            case 'Enter':
                ...
                if (nextButton && nextButton.style.display !== 'none' && !nextButton.disabled) {
                    nextButton.click();
                } else if (submitButton && submitButton.style.display !== 'none' && !submitButton.disabled) {
                    submitButton.click();
                }
                break;
        }
    ...
    function handleNextButtonClick() {
        if (userAnswers[currentQuestionIndex] === null && testWordsData.length > 0) {
    ...
    function initializeTimerAndSubmit() {
        ...
        testForm.addEventListener('submit', (e) => {
            ...
            if (!allAnswered && testWordsData.length > 0) {
